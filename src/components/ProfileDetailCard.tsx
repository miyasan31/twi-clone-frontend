...
import { PROFILE_TAB } from "src/constants/profile_tab";
...
export const ProfileDetailCard: VFC = () => {
	...
	return (
		<ProfileWrap>
			...
			<Flex>
				{PROFILE_TAB.map((value) => {
					const path = `/miyasan_0301/${value.path}`;
					return (
						<TabButton key={path} isActive={pathCheck(value.path)} onClick={() => handleRoute(path)}>
							{value.label}
							<TabActiveBar />
						</TabButton>
					);
